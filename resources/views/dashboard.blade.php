			...
			<div class="flex items-center space-x-3">
				...
				<div>
					...
					<p class="text-sm text-gray-500 dark:text-gray-400">
						{{ __('Comprehensive overview of relief operations and analytics') }}
					</p>
				</div>
			</div>
		...
		@if(isset($years) && isset($selectedYearId))
			...
			<div class="w-full">
				<form id="smartFilter" method="GET" action="{{ route('dashboard') }}"
					...
					<!-- Desktop Layout -->
					<div
						class="hidden sm:flex relative w-full bg-gradient-to-r from-white/80 via-white/70 to-white/80 dark:from-gray-900/80 dark:via-gray-900/70 dark:to-gray-900/80 backdrop-blur-xl border border-white/50 dark:border-gray-700/50 px-6 py-4 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300">
							...
							<div class="flex items-center gap-4 flex-1">
								<div class="flex items-center gap-4">
									...
									<div class="relative group">
										...
										<div
											class="absolute inset-0 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 rounded-2xl opacity-0 group-hover:opacity-20 transition-opacity duration-300 blur-sm">
										</div>
										...
										<div class="absolute inset-y-0 right-0 flex items-center pr-5 pointer-events-none">
											<div class="relative">
												<svg class="w-7 h-7 text-gray-400 group-hover:text-indigo-500 transition-all duration-300 transform group-hover:scale-110 group-hover:rotate-180"
												...
												<!-- Subtle glow effect -->
												<div
													class="absolute inset-0 bg-indigo-400 rounded-full opacity-0 group-hover:opacity-30 blur-md transition-opacity duration-300">
												</div>
											</div>
										</div>
										...
										<!-- Focus indicator -->
										<div
											class="absolute inset-0 rounded-2xl bg-gradient-to-r from-indigo-500/10 to-purple-500/10 opacity-0 group-focus-within:opacity-100 transition-opacity duration-300">
										</div>
									</div>
								</div>
							</div>
					...
					<!-- Mobile Layout -->
					<div
						class="block sm:hidden relative w-full bg-white/70 dark:bg-gray-900/60 backdrop-blur-xl border border-white/40 dark:border-gray-700/60 p-4">
						<div class="space-y-4">
							<!-- Economic Year Selector -->
							<div class="space-y-2">
								...
								<div class="relative">
									<svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-indigo-500"
										fill="none" stroke="currentColor" viewBox="0 0 24 24">
										<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
				...
				<div class="px-4 sm:px-6 py-4 border-b border-gray-200 dark:border-gray-700">
						...
						<div class="flex items-center gap-3">
							...
							<div>
								<h3 class="text-lg font-semibold text-gray-900 dark:text-white">
									{{ __('Allocation by Relief Type') }}
								</h3>
								<p class="text-sm text-gray-500 dark:text-gray-400">
									{{ __('Allocated vs Used vs Available for the selected year') }}
								</p>
							</div>
						</div>
					...
					<div class="block sm:hidden space-y-4">
						...
						<div class="flex flex-col gap-3">
							...
							<div class="space-y-2">
								<label
									class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Sort by') }}</label>
					...
					<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
						...
						@empty
							<div class="col-span-full text-center text-sm text-gray-500 dark:text-gray-400">
								{{ __('No allocations found for the selected year.') }}
							</div>
						@endforelse
					</div>
				</div>
		...
		<div id="application-status"
			...
			<div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
				<div class="flex items-center gap-3">
					...
					<div>
						<h3 class="text-lg font-semibold text-gray-900 dark:text-white">
							{{ __('Application Status Overview') }}
						</h3>
						<p class="text-sm text-gray-500 dark:text-gray-400">
							{{ __('Current status distribution of relief applications') }}
						</p>
					</div>
				</div>
				<div class="flex items-center gap-2">
			...
			<div class="flex flex-col lg:flex-row lg:justify-between lg:items-center space-y-6 lg:space-y-0">
				<div class="flex-1">
					...
					<p class="text-indigo-100 text-lg {{ app()->isLocale('bn') ? 'font-sans' : '' }}">
						{{ __('Access comprehensive distribution analysis and detailed reporting tools') }}
					</p>
				</div>
			...
			<div
				...
				<div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
					<div class="flex items-center gap-3">
						...
						<div>
							<h3 class="text-lg font-semibold text-gray-900 dark:text-white">
								{{ __('Project × Upazila Distribution') }}
							</h3>
					...
					<div class="flex items-center gap-2">
						<span
							...
							@bn($stats['allUpazilas']->count() ?? 0) {{ __('upazilas') }}
						</span>
					</div>
				</div>
				<div class="p-6">
					<div class="space-y-4">
						...
						<div class="space-y-3">
							<h4 class="text-sm font-medium text-gray-900 dark:text-white">
								{{ __('Application Distribution by Upazila') }}
							</h4>
			...
			<div
				...
				<div class="p-6">
					<div class="space-y-4">
						...
						<div class="space-y-2">
							<h4 class="text-sm font-medium text-gray-900 dark:text-white">
								{{ __('Top Duplicate Organizations') }}
							</h4>
							@if($stats['duplicateAllocations'] && $stats['duplicateAllocations']->count() > 0)
			...
			<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
				<div
					...
					<div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
						<h3 class="text-lg font-semibold text-gray-900 dark:text-white">
							{{ __('Unserved Upazilas (Selected Zilla & Year)') }}
						</h3>
					</div>
					<div class="p-6">
						@php
				...
				<div
					...
					<div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
						<h3 class="text-lg font-semibold text-gray-900 dark:text-white">
							{{ __('Unserved Unions (Selected Zilla & Year)') }}
						</h3>
					</div>
					<div class="p-6">
						@php
