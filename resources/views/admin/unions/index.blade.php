	...
	<x-slot name="header">
		<div class="flex flex-col lg:flex-row lg:justify-between lg:items-center space-y-4 lg:space-y-0">
			<div class="flex items-center space-x-3">
				...
				<div>
					<h1 class="text-2xl font-bold text-gray-900 dark:text-white">Unions Management</h1>
					<p class="text-sm text-gray-500 dark:text-gray-400">Manage administrative unions and their wards</p>
				</div>
			</div>
	...
	<div class="space-y-6">
				...
				<div class="flex items-center justify-between">
					<div class="flex items-center space-x-3">
						...
						<div>
							...
							<p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $unions->total() }}</p>
						</div>
					</div>
				</div>
			</div>
				...
				<div class="flex items-center justify-between">
					<div class="flex items-center space-x-3">
						...
						<div>
							...
							<p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $unions->where('is_active', true)->count() }}</p>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 hover:shadow-md transition-shadow duration-200">
				<div class="flex items-center justify-between">
					<div class="flex items-center space-x-3">
						...
						<div>
							<p class="text-sm font-medium text-gray-500 dark:text-gray-400">Upazilas</p>
							<p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $unions->unique('upazila.id')->count() }}</p>
						</div>
					</div>
				</div>
			</div>
				...
				<div class="flex items-center justify-between">
					<div class="flex items-center space-x-3">
						...
						<div>
							...
							<p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $unions->sum('wards_count') }}</p>
						</div>
					</div>
				</div>
			</div>
		...
		<div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
			<div class="flex items-center justify-between mb-4">
				<h3 class="text-lg font-semibold text-gray-900 dark:text-white">Filter Unions</h3>
		...
		<div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
			<div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
				<div class="flex items-center justify-between">
					<div class="flex items-center space-x-3">
						<h3 class="text-lg font-semibold text-gray-900 dark:text-white">Unions</h3>
						<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800 dark:bg-orange-900 dark:text-orange-200">
							{{ $unions->total() }} total
						</span>
					</div>
					<div class="flex items-center space-x-2">
						<span class="text-sm text-gray-500 dark:text-gray-400">Showing {{ $unions->firstItem() ?? 0 }} to {{ $unions->lastItem() ?? 0 }} of {{ $unions->total() }}</span>
					</div>
				</div>
			</div>
				...
				<table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
					<thead class="bg-gray-50 dark:bg-gray-800">
						<tr>
							...
							<th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Upazila</th>
							...
							<th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Wards</th>
						...
						<tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors duration-150">
							<td class="px-6 py-5 whitespace-nowrap">
								<div class="flex items-center">
									...
									<div class="ml-4">
										<div class="text-sm font-semibold text-gray-900 dark:text-white">{{ $union->name }}</div>
									</div>
								</div>
							</td>
							<td class="px-6 py-5 whitespace-nowrap">
							...
							<td class="px-6 py-5 whitespace-nowrap">
								<div class="text-sm font-medium text-gray-900 dark:text-white">{{ $union->upazila->name }}</div>
							</td>
							<td class="px-6 py-5 whitespace-nowrap">
								<div class="text-sm font-medium text-gray-900 dark:text-white">{{ $union->upazila->zilla->name }}</div>
							</td>
							<td class="px-6 py-5 whitespace-nowrap">
							...
							<td class="px-6 py-5 whitespace-nowrap">
								<div class="text-sm font-medium text-gray-900 dark:text-white">{{ $union->wards_count }}</div>
							</td>
							<td class="px-6 py-5 whitespace-nowrap">
								@if($union->is_active)
							...
							<td class="px-6 py-5 whitespace-nowrap text-right text-sm font-medium">
								<div class="flex justify-end space-x-2">
									...
									<form action="{{ route('admin.unions.destroy', $union) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this union?')">
										@csrf
										@method('DELETE')
										<button type="submit" 
						...
						<tr>
							<td colspan="8" class="px-6 py-12 text-center">
								<div class="flex flex-col items-center">
									...
									<h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">No unions found</h3>
									<p class="text-gray-500 dark:text-gray-400">Get started by creating your first union.</p>
