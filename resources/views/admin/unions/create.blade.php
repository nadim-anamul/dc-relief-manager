<x-main-layout>
	<x-slot name="header">
		<div class="flex items-center">
			...
			<h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('নতুন ইউনিয়ন তৈরি করুন') }}</h1>
		</div>
	</x-slot>
	...
	<div class="max-w-2xl mx-auto">
		<div class="card">
			<div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
				<h3 class="text-lg font-medium text-gray-900 dark:text-white">{{ __('ইউনিয়ন তথ্য') }}</h3>
			</div>
			<div class="p-6">
				<form action="{{ route('admin.unions.store') }}" method="POST" class="space-y-6" x-data="unionForm()">
					@csrf
					...
					<div>
						...
						<input type="text" 
							...
							placeholder="{{ __('ইউনিয়নের নাম লিখুন') }}"
							required>
						@error('name')
							<p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
						@enderror
					...
					<div>
						...
						<input type="text" 
							...
							placeholder="{{ __('অনন্য কোড লিখুন (যেমন, BOG_MUNI)') }}"
							required>
						@error('code')
							<p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
						@enderror
