<x-main-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Edit Relief Item') }}</h1>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="type" 
                            ...
                            <option value="monetary" {{ old('type', $reliefItem->type) === 'monetary' ? 'selected' : '' }}>{{ __('Monetary') }}</option>
                            <option value="food" {{ old('type', $reliefItem->type) === 'food' ? 'selected' : '' }}>{{ __('Food') }}</option>
                            <option value="medical" {{ old('type', $reliefItem->type) === 'medical' ? 'selected' : '' }}>{{ __('Medical') }}</option>
                            <option value="shelter" {{ old('type', $reliefItem->type) === 'shelter' ? 'selected' : '' }}>{{ __('Shelter') }}</option>
                            <option value="other" {{ old('type', $reliefItem->type) === 'other' ? 'selected' : '' }}>{{ __('Other') }}</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select id="unit" 
                            ...
                            <option value="BDT" {{ old('unit', $reliefItem->unit) === 'BDT' ? 'selected' : '' }}>{{ __('BDT (৳)') }}</option>
                            <option value="metric_ton" {{ old('unit', $reliefItem->unit) === 'metric_ton' ? 'selected' : '' }}>{{ __('Metric Ton') }}</option>
                            <option value="kg" {{ old('unit', $reliefItem->unit) === 'kg' ? 'selected' : '' }}>{{ __('Kilogram (Kg)') }}</option>
                            <option value="liter" {{ old('unit', $reliefItem->unit) === 'liter' ? 'selected' : '' }}>{{ __('Liter') }}</option>
                            <option value="piece" {{ old('unit', $reliefItem->unit) === 'piece' ? 'selected' : '' }}>{{ __('Piece') }}</option>
                            <option value="box" {{ old('unit', $reliefItem->unit) === 'box' ? 'selected' : '' }}>{{ __('Box') }}</option>
                        </select>
                        @error('unit')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <textarea id="description" 
                                  ...
                                  placeholder="{{ __('Brief description of the relief item') }}">{{ old('description', $reliefItem->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
