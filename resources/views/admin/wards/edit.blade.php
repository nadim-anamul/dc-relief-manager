<x-main-layout>
	<x-slot name="header">
		<div class="flex items-center">
			...
			<h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Ward: {{ $ward->name }}</h1>
		</div>
	</x-slot>
	...
	<div class="max-w-2xl mx-auto">
		<div class="card">
			...
			<div class="p-6">
				<form action="{{ route('admin.wards.update', $ward) }}" method="POST" class="space-y-6" x-data="wardForm()">
					@csrf
					@method('PUT')
					...
					<div>
						...
						<select name="zilla_id" 
							...
							@foreach($zillas as $zilla)
								<option value="{{ $zilla->id }}" {{ old('zilla_id', $ward->union->upazila->zilla_id) == $zilla->id ? 'selected' : '' }}>
					...
					<div>
						...
						<input type="text" 
							name="name_bn" 
							id="name_bn" 
							value="{{ old('name_bn', $ward->name_bn) }}"
					...
					<div>
						...
						<input type="text" 
							...
							placeholder="Enter unique code (e.g., BOG_WARD_01)"
							required>
						@error('code')
							<p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
						@enderror
					...
					<div class="flex justify-end space-x-3">
						<a href="{{ route('admin.wards.show', $ward) }}" class="btn-secondary">
							Cancel
						</a>
	...
	<script>
		function wardForm() {
			return {
				selectedZilla: '{{ old('zilla_id', $ward->union->upazila->zilla_id) }}',
				selectedUpazila: '{{ old('upazila_id', $ward->union->upazila_id) }}',
				selectedUnion: '{{ old('union_id', $ward->union_id) }}',
				...
				loadUnions() {
					if (this.selectedUpazila) {
						fetch(`/admin/unions-by-upazila/${this.selectedUpazila}`)
							...
							.then(data => {
								this.unions = data;
								// Keep current union if it belongs to selected upazila
								const currentUnion = data.find(u => u.id == this.selectedUnion);
								if (!currentUnion) {
									this.selectedUnion = '';
								}
							})
