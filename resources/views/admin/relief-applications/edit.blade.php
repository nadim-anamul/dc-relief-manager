				...
				<form action="{{ route('admin.relief-applications.update', $reliefApplication) }}" method="POST" class="space-y-8" x-data="approvalForm()">
					...
					<div class="border-b border-gray-200 dark:border-gray-700 pb-8">
						...
						<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
							<div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 {{ app()->isLocale('bn') ? 'font-sans' : '' }}">{{ __('Applicant Information') }}</label>
								<div class="p-3 bg-gray-50 dark:bg-gray-700 rounded-md">
									@if($reliefApplication->application_type === 'individual')
										<div class="flex items-center mb-2">
											...
											<p class="text-sm font-medium text-gray-900 dark:text-white">{{ $reliefApplication->applicant_name }}</p>
										</div>
										<p class="text-xs text-gray-500 dark:text-gray-400 {{ app()->isLocale('bn') ? 'font-sans' : '' }}">{{ __('Individual Application') }}</p>
									@else
										<div class="flex items-center mb-2">
											...
											<p class="text-sm font-medium text-gray-900 dark:text-white">{{ $reliefApplication->organization_name }}</p>
										</div>
